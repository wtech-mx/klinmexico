    ...
    <div class="d-flex justify-content-center">
        <div class="limit_ticket">
                <p class="text-dark">
                    ...
                    <strong> Correo electronico: </strong> {{ $venta->Ticket->Client->email}} <br><br>
            @if (!empty($venta->Ticket->DescripcionTicket))
                    <strong> Prenda: </strong>
                    ({{ $venta->Ticket->DescripcionTicket->color1 }},
                    {{ $venta->Ticket->DescripcionTicket->color2 }},
                    {{ $venta->Ticket->DescripcionTicket->marca }},
                    {{ $venta->Ticket->DescripcionTicket->modelo }},
                    {{ $venta->Ticket->DescripcionTicket->talla }},
                    {{ $venta->Ticket->DescripcionTicket->categoria }}, <br>
                    @if ($venta->Ticket->tint != 0)
                    <strong> Observaciones Tint: </strong> {{ $venta->Ticket->tint_descripcion }}, <br>
                    @endif
                    <strong> Observaciones del articulo: </strong> {{ $venta->Ticket->DescripcionTicket->observacion }}, <br><br>

                    <strong> Rack: </strong> {{ $venta->Ticket->rack }}
            ...
            @endif
                        ...
                        @php
                            switch($venta2->servicio_primario){
                                ...
                                case($venta2->servicio_primario == 'Protector'):
                                    $precio_primario = 55;
                                    break;
                                case($venta2->servicio_primario == 'Fixer Snkrs'):
                                    $precio_primario = 0;
                                    break;
                                case($venta2->servicio_primario == 'Bolsos'):
                                    $precio_primario = 0;
                                    break;
                                }
                    ...
                    @if (!empty($venta->Precio))
                        ...
                        <p class="text-dark text-left">
                            ...
                            <strong> Factura:  </strong> {{ $venta->Precio->factura }} <br>
                            @if ( $venta->Precio->factura == 'si')
                            {{ $venta->Ticket->Client->rfc }}<br>
                            {{ $venta->Ticket->Client->calle }}, {{ $venta->Ticket->Client->colonia }}, {{ $venta->Ticket->Client->alcaldia }}, {{ $venta->Ticket->Client->estado }}, {{ $venta->Ticket->Client->cp }} <br><br>
                            @endif
